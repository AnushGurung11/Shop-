...
def read_file(filename): 
    ...
    for product in items2D: 
         print(f"{product[0]:<30} | {product[1]:<30} | {product[2]:<30} {product[3]:<30} {product[4]:<30}")
    file.close()
...
def display_products():
    ...
    print(f"{header[0]:<30}{header[1]:<30}{header[2]:<30}{header[3]:<30}{header[4]:<30}")
...
while True: 
    ...
    try: 
        ...
        # if the user input is 2 then the following will print 
            ...
            #Taking the userinput for the product name and the quantity 
            nameOfProduct = input("Enter the name of the product : ").strip()
            quantity = int (input("Enter the Quantity : "))

            #Reading the file from the The main stock and performing corresponding action 
            items = read_products("products.txt")
            print(items)
